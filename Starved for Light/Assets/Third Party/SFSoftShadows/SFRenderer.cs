...
public class SFRenderer : MonoBehaviour {
	...
	private RenderTextureFormat lightmapFormat = RenderTextureFormat.Default;
	private RenderTextureFormat lightmapFormatHDR = RenderTextureFormat.DefaultHDR;
...
//		), Color.yellow);
	...
	private void RenderLightMap(Camera cam, Matrix4x4 extensionMatrix, RenderTexture target, List<SFLight> lights, List<SFPolygon> polys, Color ambient){
		var w = target.width;
		var h = target.height;
		...
		for(int i = 0; i < lights.Count; i++){
            ...
            //HACKHACK: Unity's deprecating naming schemes: Updated cam.hdr to cam.allowHDR
...
//					_DebugRect(cullRect, Color.red);
	...
	private void OnPreRender(){
		...
		var vpMatrix = extensionMatrix*cam.projectionMatrix*cam.worldToCameraMatrix;
	...
	private void OnPostRender(){
		...
		if(fogCheck > 0.0f){
			GL.PushMatrix(); {
				...
				// Loading an identity projection lets us take advantage of it when it's needed.
